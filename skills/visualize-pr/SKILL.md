...
- every logical part is present and its local tree contains only that part's files
- selecting a file in one part changes only that part's displayed diff
- each file preview tracks its companion story without exceeding 86% of the dynamic viewport, leaves visible breathing room above the bottom viewport edge, and remains internally scrollable; an overflowing file list has exactly one scrollbar owned by the tree viewport, while its heading and selected-file footer remain fully visible
- partial page gestures settle at logical section starts without blocking direct navigation to any part
- repeated files appear in every relevant part without leaking unrelated hunks
- inline review notes appear beneath the intended side and line, remain readable in both diff layouts and themes, and do not change the outer section snap position while the diff panel scrolls
